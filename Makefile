.PHONY: help prepare unit-test integration-test test docker-up docker-up-components docker-down docker-logs docker-logs-% docker-ps status

# Все команды pipenv/pytest выполняются из корня репозитория
PIPENV       = PIPENV_PIPFILE=config/Pipfile pipenv
PYTEST       = $(PIPENV) run pytest -c config/pyproject.toml
GENERATED    = .generated
...
COMPONENT_SERVICES = security_monitor journal navigation autopilot limiter emergensy mission_handler motors sprayer telemetry
...
help:
	...
	@echo "    make unit-test           Unit-тесты компонентов (components/*/tests/)"
	@echo "    make integration-test    Интеграционные тесты in-process (tests/integration/)"
...
unit-test:
	...
	@$(PYTEST) components/*/tests/ -v --tb=short
