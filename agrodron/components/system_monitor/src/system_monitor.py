...
from components.system_monitor import config
...
class SystemMonitorComponent(BaseComponent):
    ...
    def start(self) -> None:
        super().start()
        ok = self.bus.subscribe(self._journal_topic, self._on_journal_message)
        if not ok:
            print(f"[{self.component_id}] Warning: subscribe to journal topic failed")
        else:
            print(f"[{self.component_id}] Subscribed to journal tap: {self._journal_topic}")
    ...
    def stop(self) -> None:
        self.bus.unsubscribe(self._journal_topic)
        super().stop()
